...
| 你在做什么 | 查阅哪个模块 |
|-----------|-------------|
| 写完正文后做去AI自检 | 核心规则 -> 14种AI写作模式检测 -> 质量维度检查 |
| 改写某段AI味重的文字 | 改写范例库 + 冲突对话改写范例 |
| 检查是否用了禁用词 | 禁用词与句式速查 -> AI高频词（模式1）+ Gate G深层模式词 |
| 系统性去除整章AI味 | 系统性去AI三遍法 + Gate G深层模式清除 |
| 检查章尾是否有总结升华 | AI写作指纹 -> 章末总结体 |
| 判断情绪描写是否告知式 | Show Don't Tell原则 + 去AI味补充技法 |
| 快速扫描全章质量 | 快速自检口诀 + 质量维度检查 |
| 检查是否有意义膨胀/宣传腔/刻意换词 | Gate G1-G7 深层模式检测 |
...
## 14 种 AI 写作模式检测（7 基础 + 7 深层 Gate G）
...
### 模式 8：意义膨胀（Gate G1）
AI 给普通事件赋予宏大意义，像在写历史教科书。
- "标志着一个新时代" -> 直接写事件
- "这是一种无声的抗争" -> 用行为展示
- "见证了历史的转折" -> 写具体发生了什么
- "一切都因此改变" -> 写具体改变了什么
- **规则**：如果一句话听起来像"名人名言"，大概率需要重写

### 模式 9：刻意换词（Gate G2）
AI 的重复惩罚机制导致不停切换同义词。
- 同一角色在相邻段被称为"他/那个男人/对方/那人/这位年轻人"
- 同一情绪被写成"愤怒/恼火/暴怒/怒不可遏"轮番出现
- **规则**：人称和指代保持一致，自然的中文不怕重复"他"

### 模式 10：宣传性语言（Gate G3）
AI 用"推销腔"描写场景和人物，像在写广告文案。
- "充满活力的集市" -> "集市上人挤人"
- "引人入胜的故事" -> 直接讲故事
- "举世闻名的大师" -> 用具体事迹展示
- **规则**：删除评价性形容词，用事实和行为代替

### 模式 11：虚假范围（Gate G4）
AI 用"从X到Y"结构制造虚假广度感。
- "从街头小贩到王公贵族，无人不知" -> "城里人都知道"
- "从最微小的尘埃到最宏大的星辰" -> 删掉，空洞
- **规则**：X和Y不构成有意义的范围时，改写为直接陈述

### 模式 12：过度限定与模糊归因（Gate G5）
AI 给判断加过多限定词，或把观点归因于模糊权威。
- "或许可能大概" -> 直接说
- "据说"+"没有出处" -> 给出处或删
- "人们都说" -> 谁？给具体的人
...
- "让我们把目光转向" -> 直接写新场景
- "要理解这件事" -> 直接讲，读者自己理解
- **规则**：叙述者不需要"引导"读者，直接讲故事

### 模式 14：无灵魂写作（Gate G7）
去AI味不只是删除坏模式，还要注入个性。"干净但没有灵魂"的文字一样明显。
- 每个句子长度结构相同 -> 混合长短句
- 所有人说话方式一样 -> 区分角色语气
- 过于完美的逻辑链条 -> 允许思维跳跃
- 读起来像百科全书 -> 加入态度和矛盾
- **规则**：有态度、有节奏、有矛盾、有跳跃、有细节
...
- [ ] **对话自然度测试**：无书面语痕迹 = 通过
- [ ] **无意义膨胀**（Gate G1）：无"标志着/见证了/命运的安排"等宏大叙事腔
- [ ] **无刻意换词**（Gate G2）：同一角色/事物的指代在相邻段保持一致
- [ ] **无宣传性语言**（Gate G3）：无"充满活力/引人入胜/举世闻名"等推销腔
- [ ] **无虚假范围**（Gate G4）：无无意义的"从X到Y"结构
- [ ] **无过度限定**（Gate G5）：无"或许可能大概"三连用，无模糊归因
- [ ] **无先行预告**（Gate G6）：无"话说回来/让我们/要理解这件事"等引导语
- [ ] **有灵魂**（Gate G7）：角色有个性、节奏有变化、允许不完美
