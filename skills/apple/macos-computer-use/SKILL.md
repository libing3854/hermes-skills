...
## Interacting with Terminal TUI apps (MiMo Code, SSH, etc.)

For controlling foreground Terminal.app windows via AppleScript (sending Chinese text, interacting with TUI apps), see `references/macos-terminal-control.md`. Key points:
- Use clipboard paste for Chinese text (`set the clipboard to` + `keystroke "v" using command down`)
- `keystroke` produces garbled output for non-ASCII characters
- This approach steals focus — for background automation, use `computer_use` instead

## Interacting with Chrome browser (logged-in sessions)

For controlling the user's desktop Chrome via AppleScript, see `references/macos-chrome-control.md`. Key points:

**Three Chrome contexts — know which one you're using:**
- `mcp_chrome_devtools_*` tools → headless Chrome (NO user login, separate instance)
- `browser_*` tools → headless Browserbase Chromium (NO user login)
- AppleScript → user's actual desktop Chrome (HAS user login, cookies, subscriptions)

**If you need the user's logged-in session** (Gemini Pro, authenticated web apps), you MUST use AppleScript — the MCP and Browserbase tools cannot access it.

**One-time setup**: Chrome → 显示 → 开发者 → 允许 Apple 事件中的 JavaScript (then restart Chrome)
...
- Web automation you can do via `browser_*` tools — those use a real
  ...
  browser **for non-authenticated tasks**. However, if you need the
  user's logged-in session (subscriptions, authenticated web apps),
  `browser_*` and `mcp_chrome_devtools_*` tools won't work — they run
  separate headless instances with no access to the user's cookies.
  Use AppleScript on desktop Chrome instead (see `references/macos-chrome-control.md`).
  Reach for `computer_use` specifically when the task needs the
